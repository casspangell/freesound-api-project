...
class Ashari:
    ...
    def get_ollama_instructions(self, ashari_framework):
        """Generate instructions for Ollama based on the Ashari's cultural processing"""
        ...
        # Build the system instruction for Ollama
        instruction = {
            "system": f"""
            Interpret and respond to the following input through the cultural lens of The Ashari:

            The Ashari are a resilient culture shaped by survival, betrayal, and resilience. Their responses evolve based on interactions, becoming more trusting or more guarded depending on the nature of exchanges.
        ...
        if "thematic_interpretation" in ashari_framework:
            thematic_guidance = "THEMATIC CONSIDERATIONS:\n"
            for theme, data in ashari_framework["thematic_interpretation"].items():
                thematic_guidance += f"- {theme.capitalize()}: Approach with {data['stance']}. {data['guidance']}\n"
            
            # Append to the user message
            instruction["user"] += f"\n\n{thematic_guidance}"
        
        # Return the constructed instruction dictionary ready for Ollama
        return instruction

    import ollama

    def get_ashari_response(ashari, ashari_framework):
        """
        Get a response from Ollama using the Ashari cultural framework
        
        Args:
            ashari: The Ashari instance
            ashari_framework: The framework generated by process_input
            
        Returns:
            The text response from Ollama
        """
        # Get the Ollama instructions
        instructions = ashari.get_ollama_instructions(ashari_framework)
        
        # Call Ollama with the instructions
        response = ollama.chat(
            model="llama3.1",  # or whichever model you prefer
            messages=[
