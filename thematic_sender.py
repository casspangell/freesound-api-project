...
PERFORMANCE_MODEL_PATH = "performance_model.json"
...
class ThematicInstructionSender:
    ...
    def _monitor_timeline(self):
        ...
        # Main monitoring loop - use performance_clock instead of tracking from start time
        while self.is_running and next_point_idx < len(time_points):
            # Get current time from performance clock
            elapsed_time = get_clock().get_elapsed_seconds()
